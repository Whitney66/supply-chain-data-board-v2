(() => {
  ...
  const normalizeStoreStageMonthlyTables = () => {
    ...
    document.querySelectorAll('table').forEach(table => {
      ...
      const unit = /H$/i.test(dataRows[0]?.cells[1]?.textContent.trim() || '') ? 'H' : 'D';
      heading.textContent = heading.textContent.replace(/（天）|\(天\)|（D）|\(D\)/g, `（${unit}）`);
      if (!table.querySelector('[data-store-stage-overall]') && dataRows.length) {
        ...
        labels.forEach((label, index) => {
          ...
          cell.textContent = values.length ? `${Number((values.reduce((sum, value) => sum + value, 0) / values.length).toFixed(2))}${index === 1 || /月/.test(labels[index]) ? unit : ''}` : '-';
        });
      }
