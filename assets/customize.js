(() => {
  ...
  const normalizeTimingTargetUnits = () => {
    ...
    const format = (cell, unit) => { const m = cell.textContent.trim().match(/^(-?\d+(?:\.\d+)?)(天|H|D|H)?$/i); if (!m) return; let value = Number(m[1]); const source = /天|D/i.test(m[2] || '') ? '天' : /H|H/i.test(m[2] || '') ? 'H' : unit; if (source !== unit) value = source === '天' ? value * 24 : value / 24; cell.textContent = `${Number(value.toFixed(4))}${unit === '天' ? 'D' : 'H'}`; };
